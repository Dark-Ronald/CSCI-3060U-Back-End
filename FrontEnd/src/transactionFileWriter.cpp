...
void transactionFileWriter::run() {
	...
	while (true) {
		...
		string fileName(filePath + "\\" + to_string(fileNameTime.tm_year + 1900) + "-" + to_string(fileNameTime.tm_mon + 1) + "-" +
			to_string(fileNameTime.tm_mday) + ".atf");
		ofstream dTFWriter(fileName, ios::app);
